...
- On injecte les 4 canaux dans **ExceptionSubscriber** ainsi que mailer, CacheService, lock et l'environnement

- On injecte le cache et le répertoire `var/cache/<env>/pools/app` dans CacheService
...
services:
    ...
    App\EventSubscriber\ExceptionSubscriber:
        arguments:
            ...
            $mailer: '@mailer'
            $dedupeCache: '@App\Service\CacheService'
    ...
    App\Service\CacheService:
        arguments:
            $innerPool: '@cache.app'     # var/cache/<env>/pools/app à "auto-pruner" 
            $cacheDir: '%kernel.cache_dir%/pools/app' # répertoire réel des clés
            $lockFactory: '@lock.factory' 
```

### 3. Si on est sur Symfony 5.4 mettre la config du pools dans config/packages/cache.yaml
...
- `getItem()` excécute `pruneExpired()` puis `getItem()` de **CacheItemPoolInterface**
...
class CacheService implements CacheInterface, CacheItemPoolInterface
{
    ...
    private const PRUNE_INTERVAL_IN_MINUTES = 90;
    ...
    private const MAX_FILES_PER_RUN = 30;
    ...
    public function __construct(
     ...
     /**
     * ⚡ Throttle : on ne purge jamais plus d'une fois par le nombre de minute indiquer dans PRUNE_INTERVAL
     */
